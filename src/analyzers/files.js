...
const FILENAME_RULES = [
  ...
  { id: 'windsurf-settings', severity: 'error', label: '.windsurf/settings.json — Windsurf AI editor config, may contain tokens or paths', fix: "Add '.windsurf/' to .npmignore.", re: /(^|\/)\.windsurf\/settings\.json$/i },
  ...
  { id: 'terraform-vars', severity: 'error', label: 'Terraform variable file (.tfvars) — often contains secrets', fix: "Add '*.tfvars' to .npmignore.", re: /\.tfvars(\.json)?$/i },
  { id: 'k8s-secret', severity: 'error', label: 'Kubernetes secret manifest — may contain base64-encoded credentials', fix: 'Remove this file or add it to .npmignore.', re: /\.(secret|secrets)\.(ya?ml)$/i },
  { id: 'pypirc', severity: 'error', label: '.pypirc — PyPI auth token / credentials file', fix: "Add '.pypirc' to .npmignore.", re: /(^|\/)\.pypirc$/i },

  // Warnings
  { id: 'cursorrules', severity: 'warn', label: '.cursorrules — may embed private project context or internal instructions', fix: "Add '.cursorrules' to .npmignore if it contains internal context.", re: /(^|\/)\.cursorrules$/i },
